...
class Crawler
{
    ...
    public function __construct()
    {
        ...
        $this->fiction = __DIR__ . '/' . md5(Str::uuid()) . '.txt';
    ...
    public function spider($url, $select, $next, $chapterSelector)
    {
        ...
        if (false !== $chapterSelector) {
            ...
            if (!empty($chapter && is_array($chapter))) {
                $chapter = array_shift($chapter)->text();
                file_put_contents($this->fiction, $chapter, FILE_APPEND);
            }
        }
        $text = $document->find($select);

        if (!empty($text) && is_array($text)) {
            $text = array_shift($text)->text();
        } else {
            return false;
        }
        ...
        file_put_contents($this->fiction, $text, FILE_APPEND);
        if (false === $next) {
